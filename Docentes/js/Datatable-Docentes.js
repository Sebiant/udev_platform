    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.data && response.data.length > 0) {
          ...
          $('#editForm [name="declara_renta"]').prop(
            "checked",
            String(docente.declara_renta) === "1"
          );
          $('#editForm [name="retenedor_iva"]').prop(
            "checked",
            String(docente.retenedor_iva) === "1"
          );
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        materias.forEach((materia) => {
          ...
          const isChecked = asignadas.includes(parseInt(materia.id_modulo))
            ? "checked"
            : "";
      ...
      error: function () {
        $("#materiasContainerEditar").html(
          "<p class='text-danger'>Error al cargar materias.</p>"
        );
      },
    });
  }
